...
public class Activity_bsa_rpt extends Activity_LineChartReports {
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        ...
        switch (item.getItemId()) {
            ...
            case R.id.thirtydays:
                computeDrawThree(addDays(new Date(),-30), new Date(),lineChart,Activity_bsa_rpt.this.getString(R.string.erzielte_punkte));

                return true;
            case R.id.fourteendays:
                computeDrawThree(addDays(new Date(),-14), new Date(),lineChart,Activity_bsa_rpt.this.getString(R.string.erzielte_punkte));
                return  true;
            case R.id.sevendays:
                computeDrawThree(addDays(new Date(),-7), new Date(),lineChart,Activity_bsa_rpt.this.getString(R.string.erzielte_punkte));
                return true;
            default:
    ...
    protected void computeDrawThree(final Date Startdate, final Date EndDate, final LineChart lineChart, final String legend) {
        try {
            ...
            root.addListenerForSingleValueEvent(new ValueEventListener() {
                                                    ...
                                                    public void onDataChange(DataSnapshot dataSnapshot) {
                                                        ...
                                                        for (DataSnapshot child : dataSnapshot.getChildren()) {
                                                            ...
                                                            int i = 0;
                                                                ...
                                                                for (DataSnapshot child2L : child.getChildren()) {
                                                                    ...
                                                                    else if (child2L.getKey().equals("Sportscoring")){
                                                                        y_SportScoring += convertValue(child2L);
                                                                        flag = true;
                                                                    }
                                                                    else if(child2L.getKey().equals("Gesamtscoring"))
                                                                    {
                                                                        // Gesamtscoring
                                                            ...
                                                            if (flag) {
                                                                ...
                                                                //x  = sDate ;
                                                                max_y = Math.max(y_SportScoring, Math.max(y_Gesamt, Math.max(y_Bewegung, max_y)));
                                                                min_y = Math.min(y_SportScoring, Math.min(y_Gesamt, Math.min( min_y, y_Bewegung)));
                                                                xAXES.add(x);
                                                        ...
                                                        if (xAXES.size() != 0) {
                                                            ...
                                                            LineDataSet lineDataSet1 = new LineDataSet(yAXES_Bewegung, Activity_bsa_rpt.this.getString(R.string.bewegungswerte));
                                                            lineDataSet1.setDrawCircles(true);
                                                            lineDataSet1.setColor(Color.BLUE);

                                                            LineDataSet lineDataSet2 = new LineDataSet(yAXES_Gesamt, Activity_bsa_rpt.this.getString(R.string.gesagmtewerte));
                                                            lineDataSet2.setDrawCircles(true);
                                                            lineDataSet2.setColor(Color.RED);

                                                            LineDataSet lineDataSet3 = new LineDataSet(yAXES_Sportscoring, Activity_bsa_rpt.this.getString(R.string.sportwerte));
